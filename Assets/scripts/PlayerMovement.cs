...
//     void Mover()
// {
//     Vector3 movimento = direcaoMovimento * velocidadeAtual;

//     Vector3 velocidadeDesejada = new Vector3(movimento.x, rb.linearVelocity.y, movimento.z);
...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        targetLock = GetComponent<TargetLockSystem>(); // NOVO
    }
    ...
    void Rotacionar()
    {
        ...
        if (targetLock != null && targetLock.IsLocked) // NOVO — prioridade máxima
        {
            Vector3 direcaoAlvo = targetLock.CurrentTarget.position - transform.position;
            direcaoAlvo.y = 0f;

            if (direcaoAlvo.sqrMagnitude > 0.01f)
            {
                Quaternion rotAlvo = Quaternion.LookRotation(direcaoAlvo);
                Quaternion rotSuave = Quaternion.Slerp(transform.rotation, rotAlvo, Time.fixedDeltaTime * 12f);
                rb.MoveRotation(rotSuave);
            }
            return;
        }
